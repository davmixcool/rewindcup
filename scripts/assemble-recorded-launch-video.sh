...
if [[ -n "$MUSIC_FILE" ]]; then
  ...
  ffmpeg -hide_banner -y \
    ...
    -ss "$MUSIC_START" \
    -i "$MUSIC_FILE" \
    -filter_complex "[0:v]ass='$ROOT_DIR/scripts/rewindcup-recorded-launch.ass':fontsdir='/System/Library/Fonts',tpad=stop_mode=clone:stop_duration=0.2,format=yuv420p[v];[1:a]atrim=duration=24,asetpts=N/SR/TB,volume=0.72,afade=t=in:st=0:d=0.25,afade=t=out:st=23:d=1[a]" \
    -map "[v]" \
    -map "[a]" \
    -r 30 \
    -frames:v 720 \
