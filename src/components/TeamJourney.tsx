            ...
            <li className={`team-journey-stop result-${result.toLowerCase()} ${isCompleted ? "is-completed" : ""}`} key={match.id}>
              ...
              >
                ...
                <span className="tray-fixture-copy">
                  <span className="team-journey-stage">Stop {journeyIndex + 1} · {match.stageLabel ?? tournament.stageLabels?.[match.stage] ?? stageLabels[match.stage]} · {formatDate(match.date)}</span>
                  <strong>
